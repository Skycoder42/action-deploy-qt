...
`);
            ...
            const perl = yield io.which("perl", true);
...
`);
            ...
            yield replace_in_file_1.replaceInFile({
...
`);
            yield this.getAsset("doc", pkgDir, `Docs/Qt-${this.config.qtVersion}`);
        });
    }
...
`);
            yield this.getAsset("examples", pkgDir, `Examples/Qt-${this.config.qtVersion}`);
        });
    }
    getAsset(platform, pkgDir, subDir) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield io.mkdirP(dataDir);
            if (asZip)
                yield ex.exec("unzip", ["-qq", dlPath, "-d", dataDir], { silent: true });
            else
                yield ex.exec("tar", ["x", "-C", dataDir, "-f", dlPath], {
                    silent: true,
                });
            return dataDir;
        });
