...
export class Deployer {
  // constants
  private readonly qtIfwVer: string = "3.2.2";
  ...
  public async run(
    ...
    try {
      ...
      await uploader.generateRepos(
        "linux",
        "x64",
        Platforms.linuxPlatforms(excludes)
      );
      await uploader.generateRepos(
        "windows",
        "x86",
        Platforms.windowsPlatforms(excludes)
      );
      await uploader.generateRepos(
        "mac",
        "x64",
        Platforms.macosPlatforms(excludes)
      );
    } finally {
      await sshfs.unmount();
    }
  ...
  private createPackageConfig(qtVersion: string): PackageConfig | null {
    ...
    if (refs[1] != "tags") {
      core.warning(
        "Deployments are only run for tags. Not doing anything! Consider adding 'if: startsWith(github.ref, 'refs/tags/')' as condition to this step"
      );
      return null;
    }
  ...
  private async downloadRepogen(): Promise<string> {
    ...
    const repogen = path.join(
      ...
      this.qtIfwVer.substr(0, 3),
      "bin",
      "repogen"
    );
    if (!fs.existsSync(repogen))
      throw Error(
        `Unable to find repogen after aqt install with path: ${repogen}`
      );
