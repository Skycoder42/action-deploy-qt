...
export class Packager {
  ...
  public constructor(octokit: Octokit, pkgDir: string, config: PackageConfig) {
  ...
  public async getSources() {
    ...
    this.srcDlDir = path.join(
      await tc.extractTar(srcFile),
      `${gh.context.repo.owner}-${gh.context.repo.repo}-${gh.context.sha.substr(
        0,
        7
      )}`
    );
    ...
    this.config.config = await ConfigParser.loadConfig(
      path.join(this.srcDlDir, "deploy.json")
    );
  }
...
<Package>
    ...
    <Licenses>
        <License name="${
          this.config.config!.license.name
        }" file="LICENSE.txt" />
    </Licenses>
...
`
    ...
    await io.cp(
      path.join(this.srcDlDir, this.config.config!.license.path),
      path.join(metaDir, "LICENSE.txt")
    );
    ...
    if (this.config.config!.installs) {
      ...
      for (let eInfo of map) {
        await io.cp(
          path.join(this.srcDlDir, eInfo[0]),
          path.join(dataDir, eInfo[1]),
          {
            recursive: true,
            force: true,
          }
...
`
    ...
    const srcPath = path.join(
      srcBasePath,
      this.config.config!.title.toLowerCase()
    );
    await io.mv(this.srcDlDir, srcPath);

    core.info("    -> Downloading syncqt.pl");
    const syncQt = await tc.downloadTool(
      `https://code.qt.io/cgit/qt/qtbase.git/plain/bin/syncqt.pl?h=${this.config.qtVersion}`
    );
    ...
    for (let mod of this.config.config!.modules) {
      ...
      await ex.exec(
        ...
        [
          ...
          mod,
          "-version",
          this.config.pkgVersion.split("-")[0],
          "-outdir",
          srcPath,
          srcPath,
        ],
...
`
    ...
    const dataDir = await this.getAsset(
      platform,
      pkgDir,
      this.config.qtVersion
    );
    ...
    await replaceInFile({
      ...
      to: (match) => {
        ...
        for (let dep of depStr.split(" ")) {
          if (!dep.startsWith("-L")) depRes.push(dep);
        }
...
`
    ...
    await this.getAsset(
      "examples",
      pkgDir,
      `Examples/Qt-${this.config.qtVersion}`
    );
  }
  ...
  private async getAsset(
    ...
    const assetName = `${this.config.config!.title.toLowerCase()}-${platform}-${
      this.config.qtVersion
    }.${asZip ? "zip" : "tar.xz"}`;
